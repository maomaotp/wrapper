...
function App() {
  ...
  const TokenSwapABI = WrapperABI; // require("./abi/TokenSwap.json");
  ...
  const bridge = async () => {
    ...
    try {
      const tx = await tokenSwap!.connect(signer!).wrap(amountToBridge);
      await tx.wait();
    ...
    <>
      ...
      <Container my="16" minH="md" minW={["0", "0", "2xl", "2xl"]}>
        <FormControl>
          <FormLabel>
            <HStack>
              <Text>Enter Amount of Token to Swap</Text>
              <Spacer />
              <Text color="gray.300">
